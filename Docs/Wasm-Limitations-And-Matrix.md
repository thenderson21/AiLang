...
| vector UI syscalls (`sys.ui.*`) | native host implementation-dependent | blocked unless host implements | partial (`createWindow`/`beginFrame`/`drawRect`/`drawText`/`drawLine`/`drawEllipse`/`drawPath`/`drawImage`/`endFrame`/`present`/`waitFrame`/`closeWindow` via SVG; `pollEvent`/`getWindowSize` still `RUN101`) | partial (`createWindow`/`beginFrame`/`drawRect`/`drawText`/`drawLine`/`drawEllipse`/`drawPath`/`drawImage`/`endFrame`/`present`/`waitFrame`/`closeWindow` via SVG; `pollEvent`/`getWindowSize` still `RUN101`) |
