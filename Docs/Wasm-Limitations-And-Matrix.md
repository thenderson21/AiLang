...
| vector UI syscalls (`sys.ui.*`) | native host implementation-dependent | blocked unless host implements | partial (`createWindow`/`beginFrame`/`drawRect`/`drawText`/`drawLine`/`drawEllipse`/`drawPath`/`drawImage`/`endFrame`/`present`/`waitFrame`/`closeWindow` via SVG; `pollEvent` returns deterministic empty-event node; `getWindowSize` returns deterministic node updated by `createWindow`) | partial (`createWindow`/`beginFrame`/`drawRect`/`drawText`/`drawLine`/`drawEllipse`/`drawPath`/`drawImage`/`endFrame`/`present`/`waitFrame`/`closeWindow` via SVG; `pollEvent` returns deterministic empty-event node; `getWindowSize` returns deterministic node updated by `createWindow`) |
