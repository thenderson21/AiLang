...
| `--vm=c` bridge gate parity | in_progress | Manifest includes deterministic `DEV008` gate cases (source + bundled bytecode), execute-enabled source-bytecode bridge-compatibility diagnostics (`missing main`, `main params`, `CALL` target OOB, malformed `CALL_SYS` shapes), source-kind split behavior (Program source stays gate), and execute-enabled bundled behavior currently surfacing bridge compatibility diagnostics; `AiLang.Tests` asserts deterministic gate/bridge behavior across source/serve/embedded-bytecode/embedded-bundle entrypoints while runtime bridge remains backend-unlinked for full Program flow. |
