...
| `--vm=c` bridge gate parity | in_progress | Manifest includes deterministic `DEV008` gate cases (source + bundled bytecode) and execute-enabled source-bytecode bridge-compatibility diagnostics (`CALL` target OOB, malformed `CALL_SYS` shapes); `AiLang.Tests` asserts deterministic gate/bridge behavior across source/serve/embedded-bytecode/embedded-bundle entrypoints while runtime bridge remains backend-unlinked for full Program flow. |
