...
| VM error detail strings | in_progress | Deterministic detail channel added (`aivm_vm_error_detail`) for central failure paths, including syscall-failure `AIVMS*` details from `CALL_SYS`; opcode-by-opcode parity wording still being expanded. |
...
| UI lifecycle/window syscall parity | in_progress | Added C contracts for `ui_createWindow/ui_beginFrame/ui_endFrame/ui_pollEvent/ui_present/ui_closeWindow/ui_waitFrame`; `ui_getWindowSize` now matches C# shape (`1 int -> node`). |
