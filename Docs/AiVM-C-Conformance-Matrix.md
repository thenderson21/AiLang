...
| Memory leak threshold gate (Linux CI) | implemented | `scripts/aivm-mem-leak-check.sh` enforces RSS growth threshold and writes deterministic report artifact in `aivm-c-ci`. |
