...
| `--vm=c` bridge gate parity | in_progress | Manifest includes deterministic `DEV008` gate cases (source + bundled bytecode), and `AiLang.Tests` asserts deterministic gate behavior across source/serve/embedded-bytecode/embedded-bundle entrypoints while runtime bridge remains backend-unlinked. |
