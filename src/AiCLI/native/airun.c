...
static int emit_wasm_spa_files(const char* out_dir)
{
    ...
    }
    {
        ...
        const char* remote_client_js_tail =
            ...
            "        if (frame.type === 0x03 && frame.id === 1) { const err = decodeError(frame.payload); rejectReady(`remote handshake denied ${err.code}: ${err.message}`); return; }\n"
            "        if (frame.id === 1) { rejectReady(`remote unexpected handshake frame type ${frame.type}`); return; }\n"
