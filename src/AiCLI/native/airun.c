...
static int emit_wasm_spa_files(const char* out_dir)
{
    ...
    }
    {
        ...
        const char* main_js_head3 =
            ...
            "  const state = { host, svg, width, height, frameParts: [], nextElementId: 1, focusedTargetId: '', closed: false, resizeHandler: null, eventQueue: [], lastPolledEvent: { type: 'none', targetId: '', x: -1, y: -1, key: '', text: '', modifiers: '', repeat: false } };\n"
            ...
            "    svg.addEventListener('pointerdown', (ev) => { emitClickEvent(ev, ev); });\n"
            "  } else {\n"
            "    svg.addEventListener('click', (ev) => { emitClickEvent(ev, ev); });\n"
