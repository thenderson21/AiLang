...
static int emit_wasm_spa_files(const char* out_dir)
{
    ...
    }
    {
        ...
        const char* main_js_head2 =
            ...
            "  const state = { host, svg, width, height, frameParts: [], eventQueue: [], lastPolledEvent: { type: 'none', targetId: '', x: -1, y: -1, key: '', text: '', modifiers: '', repeat: false } };\n"
            "  const pushEvent = (evt) => { state.eventQueue.push(evt); if (state.eventQueue.length > 64) state.eventQueue.shift(); };\n"
            "  svg.addEventListener('click', (ev) => {\n"
            "    svg.focus();\n"
            "    pushEvent({ type: 'click', targetId: uiEventTargetId(ev), x: ev?.offsetX ?? 0, y: ev?.offsetY ?? 0, key: '', text: '', modifiers: uiEventModifiers(ev), repeat: false });\n"
            "  });\n"
            "  svg.addEventListener('keydown', (ev) => {\n"
            "    pushEvent({ type: 'key', targetId: uiEventTargetId(ev), x: -1, y: -1, key: uiEventKey(ev?.key), text: uiEventText(ev), modifiers: uiEventModifiers(ev), repeat: !!ev?.repeat });\n"
            "  });\n"
        ...
        const char* main_js_mid =
            ...
            "  const win = uiState.windows.get(windowId);\n"
            "  if (!win) return -1;\n"
            "  win.frameParts.push(`<line x1=\"${x1|0}\" y1=\"${y1|0}\" x2=\"${x2|0}\" y2=\"${y2|0}\" stroke=\"${xmlEscape(color)}\" stroke-width=\"${width|0}\"/>`);\n"
            "  return 0;\n"
            "};\n"
            ...
            "  const cx = (x|0) + rx;\n"
            "  const cy = (y|0) + ry;\n"
            "  win.frameParts.push(`<ellipse cx=\"${cx}\" cy=\"${cy}\" rx=\"${rx}\" ry=\"${ry}\" fill=\"${xmlEscape(color)}\"/>`);\n"
            "  return 0;\n"
            "};\n"
            ...
            "  if (sw > 0) {\n"
            "    win.frameParts.push(`<path d=\"${safePath}\" fill=\"none\" stroke=\"${xmlEscape(color)}\" stroke-width=\"${sw}\"/>`);\n"
            "  } else {\n"
            "    win.frameParts.push(`<path d=\"${safePath}\" fill=\"${xmlEscape(color)}\"/>`);\n"
            "  }\n"
            "  return 0;\n"
            "};\n"
            ...
            "  const win = uiState.windows.get(windowId);\n"
            "  if (!win) return -1;\n"
            "  win.frameParts.push(`<image x=\"${x|0}\" y=\"${y|0}\" width=\"${w|0}\" height=\"${h|0}\" href=\"${xmlEscape(src)}\"/>`);\n"
            "  return 0;\n"
            "};\n"
